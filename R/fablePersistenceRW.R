...
RW_daily_forecast <- function(site, var, h, depth,
  ...
  forecast_starts <- targets %>%
    ...
    dplyr::filter(datetime < ref_date) |> 
    # Start the day after the most recent non-NA value
    dplyr::summarise(start_date = max(datetime) + lubridate::days(1)) %>% # Date
    dplyr::mutate(h = (ref_date - start_date) + h) %>% # Horizon value
    dplyr::ungroup()
  ...
  targets_use <- targets %>%
    dplyr::filter(site_id == site,
                  ...
                  depth_m == depth,
                  datetime < ref_date) %>%
    tsibble::as_tsibble(key = c('variable', 'site_id', 'depth_m'), index = 'datetime') %>%
    # add NA values up to today (index)
    tsibble::fill_gaps(.end = ref_date) %>%
